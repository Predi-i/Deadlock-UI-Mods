...
// text/html), while graph.mapillary.com keeps working. Verify from the network doing the build, so a full
    ...
    if (verifyOnly) {
        ...
        if (tileIntercepted) {
            console.error("\n" + tileIntercepted + " tile requests were answered with HTML/JSON " +
                ...
                "network with direct Mapillary access, or use --resolve-only. Refusing to overwrite the pool with a partial sweep.");
            process.exit(1);
        }
