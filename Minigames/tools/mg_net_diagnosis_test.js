 ...
 * quick match, GeoGuesser and Pixel Battle all timing out - while the VPS was serving
 ...
 *
 * The discriminator is free, because the mod already talks to two unrelated hosts: the relay (a raw
 * IP, no DNS, Let's Encrypt short-lived IP cert) and raw.githubusercontent.com (needs DNS, entirely
 * different chain) for the update check. A down relay cannot stop a GitHub PNG from loading. So
...
function loadNet($) {
    for (const rel of ["mg_sound.js", "mg_net.js"]) {
        new Function("$", fs.readFileSync(path.join(SCRIPTS, rel), "utf8"))($);
    }
}
...
// Two unrelated hosts failing IS the local-blocking case, and it must be reported as such.
{
    const h = makeHarness();
    loadNet(h.$);
    h.$.MG.Api.ping(() => {}, () => {});                       // the relay (raw IP)
    h.run(60000, null);
