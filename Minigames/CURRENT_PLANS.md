...
## Сессия 2026-07-13 — замечания мейнтейнера (7 пунктов) — ✅ ВСЕ СДЕЛАНО

1. **п3 — лишний крестик на карточках** — ✅ `32b18e9` (потом badge убран целиком в `4e824a8`).
2. **п4 — Select-Multiple не выделял игры** — ✅ `404a149` + `4e824a8` (акцентная рамка `.mg-ticked`, без кружка).
3. **п6 — фишки Connect4 поверх платы** — ✅ `4c3ea7e` (передний слой синих колец `.mg-cf-front`).
4. **п7 — фишка Connect4 зависала перед приземлением** — ✅ `1c42149` (cubic-bezier вместо ease-in).
5. **п5 — панель прыгала между вью** — ✅ `2f73ac1` (фикс. высота `.mg-detail` 430px).
6. **п2 — дропдаун UI-scale** — ✅ `82ca0c3` (native DropDown, отклонён) → `4e824a8` (кастомный button+popup).
   - Native `DropDown` тащил базовый стиль игры (бумажная текстура, «…», сдвиг close-X) — записано в
     ARCHITECTURE §6 trap 11-12.
   - Пикселизация при увеличении (растровый scale2d) — **отложено мейнтейнером** (нужен 2× арт .vtex).
7. **п5-доп — дурак не тикался в Select-Multiple** — ✅ `4e824a8` (добавлен в MULTI_GAME_IDS).
8. **п1 — обновить ARCHITECTURE.md** — ✅ `a7038b6` (§8.7 Connect Four, §3 file map, §6 traps 11-13).
